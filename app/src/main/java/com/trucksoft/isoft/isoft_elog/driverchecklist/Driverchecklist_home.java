...
public class Driverchecklist_home extends FragmentActivity implements View.OnClickListener {
    Driverchecklist_receyadap adapter;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.driverchecklist_home);
        contexts=this;
...
txtybe=findViewById(R.id.dds);
        ...
        adapter = new Driverchecklist_receyadap(contexts, movies);


        adapter.setLoadMoreListener(new Driverchecklist_receyadap.OnLoadMoreListener() {
            @Override
        ...
        txtybe.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                watchYoutubeVideo(contexts,"0a_3p75dUM4");
            }
        });
...
deleteCache(contexts);
                Intent mIntent = new Intent(Driverchecklist_home.this,
...
//Log.e("kd",""+datess);
      ...
      //  String baseurl=pref.getString(Constant.BASE_URL_IMAGE);
        //clearData();
        Log.e("urldz","http://eld.e-logbook.info/elog_app/driver_checklist_home_recy.php?index="+index+"&page="
        +page+"&driver="+pref.getString(Constant.DRIVER_ID)+"&cc="+pref.getString(Constant.COMPANY_CODE)+"&typefld=driver");
        Call<List<Checklist_model>> call = api.getMovies(index,page,pref.getString(Constant.DRIVER_ID),pref.getString(Constant.COMPANY_CODE),"driver",""+lat,""+lon,""+straddress,""+strstate,"DVIR");

        call.enqueue(new Callback<List<Checklist_model>>() {
            @Override
            public void onResponse(Call<List<Checklist_model>> call, Response<List<Checklist_model>> response) {
            ...
            @Override
            public void onFailure(Call<List<Checklist_model>> call, Throwable t) {
...
//Log.e("call","loadmpre");
        ...
        movies.add(new Checklist_model("load"));
        ...
        call.enqueue(new Callback<List<Checklist_model>>() {
            ...
            public void onResponse(Call<List<Checklist_model>> call, Response<List<Checklist_model>> response) {
                ...
                if(response.isSuccessful()){
                    ...
                    List<Checklist_model> result = response.body();
                    if(result.size()>0){
                        //add loaded data
